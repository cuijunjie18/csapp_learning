 ...
 ********************************************************************/
 ...
 static int test_dim_smooth[] = {32, 64, 128, 256};
 ...
 static int check_rotate(int dim) 
 {
     ...
     for (j = 0; j < dim; j++) 
         ...
         res_bad = result[RIDX(j,dim-1-i,dim)];
         }
 ...
 void print_global_array() {
     ...
     for (int i = 0; i < ARRAY_DIM; i++) {
         for (int j = 0; j < ARRAY_DIM; j++) {
             printf("%d ", kernel_array[i][j]);
         }
         printf("\n");
     }
 }
 
 // 生成从 n 个位置中选择 k 个位置的所有组合
 void generate_combinations(int n, int k, int *combination, int start, int index, int *combinations, int *combination_count) {
     if (index == k) {
         int num = 0;
         // 将当前组合转换为二进制数字
         for (int i = 0; i < k; i++) {
             num |= (1 << combination[i]);  // 设置对应位置的 1
         }
         combinations[*combination_count] = num;
         (*combination_count)++;
         return;
     }
     ...
     for (int i = start; i < n; i++) {
         combination[index] = i;
         generate_combinations(n, k, combination, i + 1, index + 1, combinations, combination_count);
     }
 }
 
 // 根据组合生成 kernel_array
 void initialize_global_array(int num) {
     // 清空 kernel_array
     for (int i = 0; i < ARRAY_DIM; i++) {
         for (int j = 0; j < ARRAY_DIM; j++) {
             kernel_array[i][j] = 0;
     ...
     // 将 num 转为二进制，填充到 kernel_array 中
     for (int i = 0; i < TOTAL_BITS; i++) {
         if ((num >> (TOTAL_BITS - 1 - i)) & 1) {
             kernel_array[i / ARRAY_DIM][i % ARRAY_DIM] = 1;
         }
     }
 }
 
 void find_and_initialize(char* student_id) {
     // 提取学生学号后四位
     char last_four[5];
     strncpy(last_four, student_id + strlen(student_id) - 4, 4);
     last_four[4] = '\0';  // 确保字符串结尾有一个结束符
 
     // 将后四位转换为整数 x
     int x = 0;
     for (int i = 0; i < 4; i++) {
         if (isdigit(last_four[i])) {
             x = x * 10 + (last_four[i] - '0');
         } else if (isalpha(last_four[i])) {
             x = x * 10 + abs((tolower(last_four[i]) - 'a')) % 10;
         }
     }
 
     // 计算余数 y
     int y = x % COMBINATIONS;
     int combinations[COMBINATIONS];
     int combination[ONES_COUNT];
     int combination_count = 0;
 
     // 生成所有 9 位二进制数的组合，5 个 1，4 个 0
     generate_combinations(TOTAL_BITS, ONES_COUNT, combination, 0, 0, combinations, &combination_count);
     ...
     // 找到第 y + 1 大的数字
     int selected_number = combinations[y];
 
     // 使用选中的数字初始化 kernel_array
     initialize_global_array(selected_number);
 }
 
 
 int main(int argc, char *argv[])
 {
     find_and_initialize(student.id);
     int i;
     ...
     // if (strcmp("bovik", student.student) == 0) {
     ...
     printf("naive CPEs");
     for (i = 0; i < DIM_CNT; i++) {
     printf("\t%.1f", benchmarks_rotate[0].cpes[i]);
     }
     printf("\n");
 
     printf("your CPEs");
     for (i = 0; i < DIM_CNT; i++) {
     printf("\t%.1f", benchmarks_rotate[1].cpes[i]);
     }
     printf("\n");
     ...
     printf("Speedup\t");
     for (i = 0; i < DIM_CNT; i++) {
         if (benchmarks_rotate[1].cpes[i] > 0.0) {
         ratio = benchmarks_rotate[0].cpes[i]/
             benchmarks_rotate[1].cpes[i];
         }
         else {
             ratio = 0;	
         //printf("Fatal Error: Non-positive CPE value...\n");
     ...
     printf("naive CPEs");
     for (i = 0; i < DIM_CNT; i++) {
     printf("\t%.1f", benchmarks_smooth[1].cpes[i]);
     }
     printf("\n");
 
     printf("your CPEs");
     for (i = 0; i < DIM_CNT; i++) {
     printf("\t%.1f", benchmarks_smooth[0].cpes[i]);
     }
     printf("\n");
     ...
     printf("Speedup\t");
     for (i = 0; i < DIM_CNT; i++) {
         if (benchmarks_smooth[0].cpes[i] > 0.0) {
         ratio = benchmarks_smooth[1].cpes[i]/
             benchmarks_smooth[0].cpes[i];
         }
         else {
             ratio = 0;
         //printf("Fatal Error: Non-positive CPE value...\n");
